...
from vllm_hust_kv_tiering.residency import BlockResidencyTracker
...
class SessionLifecycleManager:
    ...
    def __init__(self, config: LifecycleConfig):
        ...
        # Avoid scanning every retained session on every scheduler step.  TTL
        # deadlines change only on request lifecycle transitions, so keep the
        # next due deadline as a cheap admission gate for expiration work.
        self._next_expiration_at: float | None = None
    ...
    def on_request_finished(
        ...
        *,
        ...
        if not state.active_req_ids:
            ...
            if state.ttl_deadline is not None and (
                self._next_expiration_at is None
                or state.ttl_deadline < self._next_expiration_at
            ):
                self._next_expiration_at = state.ttl_deadline
            if self.enabled:
    ...
    def expire_idle_sessions(
        ...
        *,
        ...
        if (
            self._next_expiration_at is None
            or self._next_expiration_at > now
        ):
            return ExpirationResult()
    ...
    def has_expiration_due(self) -> bool:
        """Return whether TTL expiration needs scheduler-step work now."""
        return (
            self._next_expiration_at is not None
            and self._next_expiration_at <= time.monotonic()
        )
    ...
    def has_pending_expiration(self) -> bool:
        return self._next_expiration_at is not None
    ...
    def _refresh_next_expiration(self) -> None:
        if self.config.idle_ttl_sec <= 0:
            self._next_expiration_at = None
            return
        self._next_expiration_at = min(
            (
                state.ttl_deadline
                for state in self._sessions.values()
                if state.is_idle and state.ttl_deadline is not None
            ),
            default=None,
        )
