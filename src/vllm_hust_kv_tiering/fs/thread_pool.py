...
from vllm_hust_kv_tiering.base import JobId
...
class JobState:
    ...
    def task_done(self, success: bool) -> tuple[bool, bool]:
        ...
        with self._lock:
            ...
            if not success:
                self._success = False
            completed = self._completed == self._n_tasks
            success = self._success
        if completed and self._on_complete is not None:
            try:
                self._on_complete(success)
            except Exception:
                logger.exception("Job %s completion callback failed", self._job_id)
                success = False
...
class DualQueueThreadPool:
    ...
    def enqueue_store(
        ...
        state = JobState(job_id, n_tasks, on_complete)
        with self._condition:
            self._inflight_jobs += 1
            for fn in tasks:
